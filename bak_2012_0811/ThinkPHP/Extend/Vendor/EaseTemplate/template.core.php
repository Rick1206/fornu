...
<!-- IF(\$rosseTemplate3_Cache) -->{run:@eval('echo \"'.\$rosseTemplate3_Cache.'\";')}<!-- END -->
<!-- END -->";
	...
	function CompilePHP(
		...
		if ($content){
			...
			if($this->RunType=='Cache' && !is_file($this->CacheDir.'index.htm')){
				$rosse_name   = 'Ease Template!';
				$rosse_base   = "<title>$rosse_name</title><a href='http://www.systn.com'>$rosse_name</a>";
				$this->writer($this->CacheDir.'index.htm',$rosse_base);
				$this->writer($this->CacheDir.'index.html',$rosse_base);
				$this->writer($this->CacheDir.'default.htm',$rosse_base);
			}
			...
			$this->writer($this->FileName($wfile,$this->TplID) ,$this->Hacker.'$rosseTemplate3_Cache = \''.$content.'\';');
		}
	}
				...
				if ($Filename=='include_page'){
					...
					if (@is_file($CacheFile)){
						...
						ob_implicit_flush(0);
							...
							//获得列表文件
							if($rosseTemplate3_Cache!=''){
								unset($ks,$vs);
								preg_match_all('/<\!-- ET\_inc\_cache\[(.+?)\] -->/',$rosseTemplate3_Cache, $IncFile);
								...
								@eval('echo "'.$rosseTemplate3_Cache.'";');
								$contents = ob_get_contents();
								ob_end_clean();
								return $contents;
							}
