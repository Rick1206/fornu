...
$(".u1 > a").hover(function(){
						...
						if(_inter)
						{
						 ...
						 _inter = setTimeout(function(){ hide(); },1000);
...
//update by rick 2012 0811
$('.map-icon').bind({  
    'mouseover':ani
});
function ani(e){
	//$(e.target).unbind('mouseover', ani);
	$(e.target).unbind('mouseover', ani).animate({top:"-="+10},300,function(){
			$(e.target).animate({top:"+="+10},300).bind('mouseover', ani);	
	});
}
